...
class TagsField extends Component {
    ...
    render () {
        ...
        let { name, label, classes, data, formValues, profile } = this.props;

        let selectedTags = _.get(formValues, name, []);
        let profileTags = _.get(profile, name, []);
        
        let tags = _.isEmpty(profileTags) ? _.union(selectedTags, data) : _.union(selectedTags, profileTags);
